...
export default function PricingPage() {
    ...
    // Fetch Services Data using the API helper
    useEffect(() => {
    ...
    // Find currently selected active service
    ...
    return (
        <div className="w-full min-h-screen pt-30 bg-[#EDE8F5] dark:bg-[#000000] text-gray-900 dark:text-gray-100 py-16 px-4 md:px-8 lg:px-12 transition-colors duration-500">
            ...
            {/* 2. Service Tabs (Horizontal Scrollable for All Services) */}
                        ...
                        {services.map((service) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`relative flex items-center gap-2.5 px-5 py-3 rounded-xl font-semibold text-sm transition-all whitespace-nowrap shrink-0 ${isActive
                                        ? 'text-white'
                    ...
                    >
                        {activeService.packages.map((pkg) => {
                            ...
                            return (
                                <div
                                    key={pkg.tier}
                                    className={`relative flex flex-col justify-between rounded-3xl p-6 md:p-8 transition-all duration-300 border ${isPopular
                                        ? 'bg-white dark:bg-[#09090b] border-[#3D52A0] shadow-2xl shadow-[#3D52A0]/20 lg:-translate-y-2'
                                        : 'bg-white/80 dark:bg-[#09090b]/80 border-gray-200 dark:border-gray-800 shadow-xl'
                                        }`}
                                >
                                    {/* Popular Badge */}
                                    {isPopular && (
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 min-h-[40px] break-words">
                                            {pkg.subtitle}
                                        </p>
                                        ...
                                        <div className="flex flex-wrap items-center gap-4 py-3 px-4 rounded-xl bg-gray-50 dark:bg-[#0f172a] mb-6 text-xs font-semibold text-gray-600 dark:text-gray-300">
                                            <div className="flex items-center gap-1.5 shrink-0">
                                                <Clock size={15} className="text-[#3D52A0]" />
                                                <span>{pkg.deliveryDays} Days Delivery</span>
                                            </div>
                                            <div className="flex items-center gap-1.5 shrink-0">
                                                <RotateCcw size={15} className="text-[#3D52A0]" />
                                            ...
                                            {pkg.features.map((feat, idx) => (
                                                <div key={idx} className="flex items-start justify-between gap-2 group relative">
                                                    <div className="flex items-start gap-3 w-full">
                                                        {feat.isIncluded ? (
                                                            <div className="shrink-0 mt-0.5 p-0.5 rounded-full bg-green-100 dark:bg-green-900/40 text-green-600 dark:text-green-400">
                                                                <Check size={14} />
                                                            </div>
                                                        ) : (
                                                            <div className="shrink-0 mt-0.5 p-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-400">
                                                                <X size={14} />
                                                            </div>
                                                        )}
                                                        <span className={`text-sm break-words ${feat.isIncluded ? 'text-gray-800 dark:text-gray-200 font-medium' : 'text-gray-400 line-through'}`}>
                                                            {feat.name}
                                                        </span>
                                                    </div>
                                                    ...
                                                    {feat.tooltip && (
                                                        <div
                                                            ...
                                                            onMouseEnter={() => setHoveredTooltip(`${pkg.tier}-${idx}`)}
                                    ...
                                    {/* Order Button */}
                                    <button className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${isPopular
                                        ? 'bg-[#3D52A0] hover:bg-[#2d3d7a] text-white shadow-lg shadow-[#3D52A0]/30'
                                        : 'bg-gray-100 dark:bg-[#0f172a] hover:bg-[#3D52A0] hover:text-white text-gray-900 dark:text-white'
                                        }`}>
            ...
            {/* 4. Enterprise Custom Quote Banner */}
            <div className="w-full mt-20">
                <div className="w-full bg-white dark:bg-[#09090b] border border-gray-200 dark:border-gray-800 rounded-3xl p-8 lg:p-12 shadow-2xl flex flex-col lg:flex-row items-center justify-between gap-8 relative overflow-hidden">
                    ...
                    <div className="max-w-2xl text-center lg:text-left">
                        <h2 className="text-2xl md:text-3xl lg:text-4xl font-extrabold mb-4 break-words">
                            Need Something Unique or Enterprise-Grade?
                        </h2>
                        <p className="text-gray-600 dark:text-gray-400 text-base md:text-lg break-words">
                            If our fixed packages don't fit your exact specs, request a tailored quote. We build high-security, custom architectures with milestone-based advance payments.
                        </p>
                    </div>

                    <Link
                        href="/quote"
                        className="shrink-0 bg-[#3D52A0] hover:bg-[#2d3d7a] text-white px-8 py-5 rounded-2xl font-bold text-lg flex items-center gap-3 transition-all shadow-xl shadow-[#3D52A0]/30"
                    >
